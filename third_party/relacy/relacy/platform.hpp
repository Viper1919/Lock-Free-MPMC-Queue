...
// LOCAL PATCH (see ../VENDORED.md): the upstream code passed the
// fiber_ctx_t* through makecontext's variadic int arguments, which
// truncates a 64-bit pointer on LP64 AArch64 (Darwin) and crashes. The
// pointer is handed off through this static instead — safe because the
// simulation is single-threaded and create_fiber swaps into the new
// fiber synchronously before it can be called again.
static fiber_ctx_t* fiber_start_ctx_;

static void fiber_start_fnc()
{
    fiber_ctx_t* ctx = fiber_start_ctx_;
...
inline void create_fiber(fiber_t& fib, void(*ufnc)(void*), void* uctx)
{
    ...
    fiber_start_ctx_ = &ctx;  // LOCAL PATCH: see fiber_start_fnc above
    makecontext(&fib.fib, (void(*)())fiber_start_fnc, 0);
